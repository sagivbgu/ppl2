...
describe('Q4 Tests', () => {
    ...
    it('parses primitive ops (extra cases)', () => {   
        ...
        expect(l2toJSResult(`(+ (+ ) (+ ))`)).to.deep.equal(makeOk(`(1 + 1)`));
        expect(l2toJSResult(`(+ (* ) (/ ))`)).to.deep.equal(makeOk(`(1 + 0)`));
        expect(l2toJSResult(`(- -3 5 -7)`)).to.deep.equal(makeOk(`(-3 - 5 - (-7))`));
        expect(l2toJSResult(`(- -3 -5 7)`)).to.deep.equal(makeOk(`(-3 - (-5) - 7)`));
    });
    ...
    it('parses equality primitive ops', () => {
        ...
        expect(l2toJSResult(`(< 1 2 3)`)).to.satisfy(isFailure);
        expect(l2toJSResult(`(> )`)).to.satisfy(isFailure);
        expect(l2toJSResult(`(> 1)`)).to.satisfy(isFailure);
        expect(l2toJSResult(`(> 1 2)`)).to.deep.equal(makeOk(`(1 > 2)`));
        expect(l2toJSResult(`(> 1 2 3)`)).to.satisfy(isFailure);
        ...
        expect(l2toJSResult(`(eq? 1 2 3)`)).to.satisfy(isFailure);
    });
    ...
    it('parses "not" primitive ops', () => {
        ...
        expect(l2toJSResult(`(not #t #t )`)).to.satisfy(isFailure);
    });
    ...
    it('parses "number?" primitive ops', () => {
        ...
        expect(l2toJSResult(`(number? 1 2)`)).to.satisfy(isFailure);
    });
    ...
    it('parses "boolean?" primitive ops', () => {
        ...
        expect(l2toJSResult(`(boolean? 1 2)`)).to.satisfy(isFailure);
    });
    ...
    it('parses "lambda" expressions', () => {
        ...
        expect(l2toJSResult(`((lambda () (* 1 2)))`)).to.deep.equal(makeOk(`(() => (1 * 2))()`));
        expect(l2toJSResult(`((lambda (x y z) (* x y z)) 3 4 5)`)).to.deep.equal(makeOk(`((x,y) => (x * y * z))(3,4,5)`));
    });
    ...
    it('parses programs', () => {
        ...
        expect(bind(parseL2(`(L2 )`), l2ToJS)).to.deep.equal(makeOk(``));
    });
